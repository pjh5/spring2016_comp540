...
try:
  ...
  print 'run the following from the pa5 directory and try again:'
...
def conv_forward_im2col(x, w, b, conv_param, drop_p=0.5):
  ...
  res = w_drop.reshape((w.shape[0], -1)).dot(x_cols) + b.reshape(-1, 1)
...
def conv_backward_im2col(dout, cache):
  ...
  based on im2col and col2im.
  """
  x, w, b, conv_param, x_cols, drops = cache
  stride, pad = conv_param['stride'], conv_param['pad']
  ...
  dw = np.multiply(dout_reshaped.dot(x_cols.T).reshape(w.shape), drops)

  dx_cols = w_drop.reshape(num_filters, -1).T.dot(dout_reshaped)
