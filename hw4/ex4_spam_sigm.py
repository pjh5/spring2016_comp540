...
Cvals = [50] #[10,30,50,100]
sigma_vals = [10 #[1,3,10,30,50,100]
...
for sigma in sigma_vals:
  print "Calculating K"
  sys.stdout.flush()
  K = np.array([utils.sigmoid_kernel(x1,x2,sigma,1) for x1 in X for x2 in X]).reshape(X.shape[0],X.shape[0])
  scaler = preprocessing.StandardScaler().fit(K)
  scaleK = scaler.transform(K)
  KK = np.vstack([np.ones((scaleK.shape[0],)),scaleK]).T
  
  Kval = np.array([utils.sigmoid_kernel(x1,x2,sigma,1) for x1 in Xval for x2 in X]).reshape(Xval.shape[0], X.shape[0])
...
svm = LinearSVM_twoclass()
svm.theta = np.zeros((X.shape[1],))
K = np.array([utils.sigmoid_kernel(x1,x2,best_sigma,1) for x1 in X for x2 in X]).reshape(X.shape[0],X.shape[0])
scaler = preprocessing.StandardScaler().fit(K)
scaleK = scaler.transform(K)
KK = np.vstack([np.ones((scaleK.shape[0],)),scaleK]).T
svm.theta = np.zeros((KK.shape[1],))
K_test = np.array([utils.sigmoid_kernel(x1,x2,best_sigma,1) for x1 in X_test for x2 in X]).reshape(X_test.shape[0],X.shape[0])
