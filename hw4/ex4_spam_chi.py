...
Cvals = [5] #[1,3,5,10,30]
sigma_vals = [0.01] #[0.0005,0.01,0.03,0.1,0.3,1,3,10,30]
...
for sigma in sigma_vals:
  print "Calculating K"
  sys.stdout.flush()
  K = utils.chi_kernel(X,X,sigma).reshape(X.shape[0],X.shape[0])
  scaler = preprocessing.StandardScaler().fit(K)
  scaleK = scaler.transform(K)
  KK = np.vstack([np.ones((scaleK.shape[0],)),scaleK]).T
  
  Kval = utils.chi_kernel(Xval,X,sigma).reshape(Xval.shape[0], X.shape[0])
...
svm = LinearSVM_twoclass()
svm.theta = np.zeros((X.shape[1],))
K = utils.chi_kernel(X,X,best_sigma).reshape(X.shape[0],X.shape[0])
scaler = preprocessing.StandardScaler().fit(K)
scaleK = scaler.transform(K)
KK = np.vstack([np.ones((scaleK.shape[0],)),scaleK]).T
svm.theta = np.zeros((KK.shape[1],))
K_test = utils.chi_kernel(X_test,X,best_sigma).reshape(X_test.shape[0],X.shape[0])
